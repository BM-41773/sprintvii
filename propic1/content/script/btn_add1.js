    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var x = 0; x < data.length; x++) {
                ...
                var html = '<img src="' + data[x].Profilepic + '"style="height:35px;" /> <label><br><hr>' + data[x].Firstname + '</label><label><br><hr>' + data[x].Surname + '</label><br><hr><input type="button" value="Suspend" id="btn_spnd" style="background-color:blue;color:white;" ><input type="button" value="Delete" id="btn_dlt" style="background-color:blue;color:white;" /><br><hr><label>day:-' + data[x].day + '</label><br><hr><label>Month:-' + data[x].month + '</label><br><hr><label>Year:-' + data[x].Year + '</label><br><hr><label>Gender:-' + data[x].Gender + '</label><br><hr><label>Email:-' + data[x].Email + '</label><input type="text" value=' + data[x].Email + ' id="email"><hr>';
                $(".PF").append(html);
            }
        }
